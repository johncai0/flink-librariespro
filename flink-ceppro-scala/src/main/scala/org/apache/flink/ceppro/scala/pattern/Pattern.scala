 ...
 */
package org.apache.flink.ceppro.scala.pattern

import org.apache.flink.ceppro
import org.apache.flink.ceppro.nfa.aftermatch.AfterMatchSkipStrategy
import org.apache.flink.ceppro.pattern.conditions.IterativeCondition.{Context => JContext}
import org.apache.flink.ceppro.pattern.conditions.{IterativeCondition, SimpleCondition}
import org.apache.flink.ceppro.pattern.{MalformedPatternException, Quantifier, Pattern => JPattern}
import org.apache.flink.ceppro.scala.conditions.Context
import org.apache.flink.ceppro.pattern
import org.apache.flink.streaming.api.windowing.time.Time

/**
  * Base class for a pattern definition.
  *
  * A pattern definition is used by [[org.apache.flink.ceppro.nfa.compiler.NFACompiler]] to create
  * a [[org.apache.flink.ceppro.nfa.NFA]].
  *
  * {{{
...
class Pattern[T , F <: T](jPattern: JPattern[T, F]) {
  ...
  def where(condition: (F, Context[F]) => Boolean): Pattern[T, F] = {
    val condFun = new IterativeCondition[F] {
      val cleanCond = ceppro.scala.cleanClosure(condition)
  ...
  def until(untilCondition: (F, Context[F]) => Boolean): Pattern[T, F] = {
    val condFun = new IterativeCondition[F] {
      val cleanCond = ceppro.scala.cleanClosure(untilCondition)
...
object Pattern {
  ...
  /**
    ...
    */
  def apply[T, F <: T](jPattern: pattern.Pattern[T, F]) = new Pattern[T, F](jPattern)
