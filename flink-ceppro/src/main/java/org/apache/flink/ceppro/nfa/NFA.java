...
package org.apache.flink.ceppro.nfa;
...
import org.apache.flink.ceppro.nfa.sharedbuffer.NodeId;
...
import static org.apache.flink.ceppro.nfa.MigrationUtils.deserializeComputationStates;

/**
 * Non-deterministic finite automaton implementation.
 *
 * <p>The {@link org.apache.flink.ceppro.operator.CepOperator CEP operator}
...
public class NFA<T> {
	 ...
	 */
	public List<Map<Tuple2<String, String>, List<T>>> process(
	...
	private List<Map<Tuple2<String, String>, List<T>>> doProcess(
			final SharedBufferAccessor<T> sharedBufferAccessor,
			final NFAState nfaState,
	...
	public static class MigratedNFA<T> {
		...
		private final org.apache.flink.ceppro.nfa.SharedBuffer<T> sharedBuffer;

		public org.apache.flink.ceppro.nfa.SharedBuffer<T> getSharedBuffer() {
			return sharedBuffer;
		}
		...
		MigratedNFA(
				...
				final org.apache.flink.ceppro.nfa.SharedBuffer<T> sharedBuffer) {
	...
	public static final class NFASerializerConfigSnapshot<T> extends CompositeTypeSerializerConfigSnapshot<MigratedNFA<T>> {
		...
		public NFASerializerConfigSnapshot(
				...
				TypeSerializer<org.apache.flink.ceppro.nfa.SharedBuffer<T>> sharedBufferSerializer) {
	...
	public static final class MigratedNFASerializerSnapshot<T> extends CompositeTypeSerializerSnapshot<MigratedNFA<T>, NFASerializer<T>> {
		...
		protected NFASerializer<T> createOuterSerializerWithNestedSerializers(TypeSerializer<?>[] nestedSerializers) {
			...
			@SuppressWarnings("unchecked")
			TypeSerializer<org.apache.flink.ceppro.nfa.SharedBuffer<T>> sharedBufferSerializer =
				(TypeSerializer<org.apache.flink.ceppro.nfa.SharedBuffer<T>>) nestedSerializers[1];

			return new NFASerializer<T>(eventSerializer, sharedBufferSerializer);
		}
	}
	...
	public static class NFASerializer<T> extends TypeSerializer<MigratedNFA<T>> {
		...
		private final TypeSerializer<org.apache.flink.ceppro.nfa.SharedBuffer<T>> sharedBufferSerializer;

		private final TypeSerializer<T> eventSerializer;

		public NFASerializer(TypeSerializer<T> typeSerializer) {
			this(typeSerializer,
				new org.apache.flink.ceppro.nfa.SharedBuffer.SharedBufferSerializer<>(
		...
		public MigratedNFA<T> deserialize(DataInputView source) throws IOException {
			...
			org.apache.flink.ceppro.nfa.SharedBuffer<T> sharedBuffer = sharedBufferSerializer.deserialize(source);
