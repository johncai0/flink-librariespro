...
package org.apache.flink.ceppro.nfa.compiler;

import org.apache.flink.api.java.tuple.Tuple2;
import org.apache.flink.ceppro.nfa.NFA;
import org.apache.flink.ceppro.nfa.State;
import org.apache.flink.ceppro.nfa.StateTransition;
import org.apache.flink.ceppro.nfa.StateTransitionAction;
import org.apache.flink.ceppro.nfa.aftermatch.AfterMatchSkipStrategy;
import org.apache.flink.ceppro.pattern.GroupPattern;
import org.apache.flink.ceppro.pattern.MalformedPatternException;
import org.apache.flink.ceppro.pattern.Pattern;
import org.apache.flink.ceppro.pattern.Quantifier;
import org.apache.flink.ceppro.pattern.Quantifier.Times;
import org.apache.flink.ceppro.pattern.conditions.BooleanConditions;
import org.apache.flink.ceppro.pattern.conditions.IterativeCondition;
import org.apache.flink.ceppro.pattern.conditions.RichAndCondition;
import org.apache.flink.ceppro.pattern.conditions.RichNotCondition;
...
public class NFACompiler {
	 ...
	 * multiple NFAs.
	 *
	 * @param patterns Definition of sequence pattern
	...
	public static <T> Map<String,NFAFactory<T>> compileFactory(
		final Map<String,Pattern<T, ?>> patterns,
		boolean timeoutHandling) {
		HashMap<String, NFAFactory<T>> stringNFAFactoryHashMap =
				new HashMap<>(patterns == null || patterns.size() == 0? 1: patterns.size());
		if (patterns == null || patterns.size() == 0) {
			// return a factory for empty NFAs
			stringNFAFactoryHashMap.put("default",new NFAFactoryImpl<>(0, Collections.<State<T>>emptyList(), timeoutHandling,"default"));
			return stringNFAFactoryHashMap;
		} else {
			patterns.forEach((k,v) -> {
				final NFAFactoryCompiler<T> nfaFactoryCompiler = new NFAFactoryCompiler<>(v);
				nfaFactoryCompiler.compileFactory();
				stringNFAFactoryHashMap.put(
						k,
						new NFAFactoryImpl<>(nfaFactoryCompiler.getWindowTime(), nfaFactoryCompiler.getStates(), timeoutHandling,k)
				);
			});
			return stringNFAFactoryHashMap;
		}
	...
	private static class NFAFactoryImpl<T> implements NFAFactory<T> {
		...
		public NFA<T> createNFA() {
			return new NFA<>(states, windowTime, timeoutHandling,key);
		}
	}
}
